...
class productQAService extends AbstractServices {
  ...
  public async getAllQuestionServiceClient(req: Request, by: string) {
    ...
    const sql = `select count(product_qa.id) as total from rrf.product_qa where product_qa.status = 'live' and product_qa. ?? = ?`;
  ...
  public async getAllQuestionServiceAdmin(req: Request) {
    ...
    if (type === "deleted") {
      sql = `select count(product_qa.id) as total from rrf.product_qa where product_qa.status = 'disabled'`;
    } else if (type === "answered") {
      sql = `select count(product_qa.id) as total from rrf.product_qa where product_qa.status = 'live' and product_qa.answer is not null`;
    } else {
      sql = `select count(product_qa.id) as total from rrf.product_qa where product_qa.status = 'live' and product_qa.answer is null`;
    }
  ...
  public async getAllQuestionOfProductForAdmin(req: Request) {
    ...
    let sql: string = `select count(product_qa.id) as total from rrf.product_qa where product_qa.product = ?`;
